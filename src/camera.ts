...
export default class Camera extends EventEmitter implements CameraState {
  ...
  /**
   * Method used to (un)zoom while preserving the position of the mouse relatively to the graph.
   *
   * @param target
   * @param dimensions
   * @param ratio
   * @return {CameraState}
   */
  getZoomToState(target: Coordinates, dimensions: Dimensions, ratio: number): CameraState {
    ...
    const graphMousePosition = this.viewportToGraph(dimensions, target);
